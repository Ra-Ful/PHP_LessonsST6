...
<script>
  function Ajax_Request(cmd, data_in, param) {
    ...
    fetch('ajax_04.php', {
      ...
      .then(data_out => {
        ...
        if (cmd == 'item_change') {
          ...
          if (param.hasAttribute('checked')) {
            param.removeAttribute('checked');
          } else {
            param.setAttribute('checked', 'checked');
          }

          param.closest("li").classList.toggle("completed");

        }

        if (cmd == 'item_all') {
          data_out.item_all.forEach((item) => item_add(item));          
        }
  ...
  todoListItem.addEventListener("change", function(event) {
    ...
    Ajax_Request('item_change', data, item);
  ...
  todoListItem.addEventListener("click", function(event) {
    ...
    if (item.classList.contains('remove')) {
      ...
      Ajax_Request('item_delete', data, item);
